...
if len(sys.argv) < 3:
    print("Usage: Python Project2.py <HostUrl> <InputFile> <QueryTerm>")
    sys.exit()
conf = (SparkConf()
        .setMaster(sys.argv[1])
        .setAppName("Project2"))
sc = SparkContext(conf=conf)
FILENAME = sc.broadcast(sys.argv[2])
QUERYTERM = sc.broadcast(sys.argv[3])
...
# Count each line by key to get the total number of documents each term appears
# Store the result in a broadcasted Dictonary
TermCountCorpus = sc.broadcast(TermFrequencyDocs.countByKey())


# Map each line into the form ((t,d),(N,M,T))
# (t,(d,N,M)) --> ((t,d),(N,M,T))
...
# Create a Boradcasted Dictonary containing all the TFIDFs for each DOCUMENT
# pertaining to the given QUERYTERM.
QTD = sc.broadcast(
    {
...
# Create a list which contains all the TERMS which are related to the QUERYTERM
# by having sharing at least one document in common.
RT = sc.broadcast(
    TFIDF.filter(
        lambda line:
...
# A dictonary containing the square root of the summation of the squared TFIDFs
# keyed by a given TERM.
CSD = sc.broadcast(
    {
...
# Compute the cos similairty for a given QUERYTERM and it's related terms by
# computing the summation of TFIDFs for the QUERYTERM and it's related terms
# who share DOCUMENTs,divided by the the product of the two values returned by
# CSD[A] * CSD[B], then sort the final results into decending order in the
# form of ((QUERYTERM,t), SIMILARITY)
